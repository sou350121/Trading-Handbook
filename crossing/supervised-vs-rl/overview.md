...
**Status:** v0.7 — Opus 手寫綜合，非摘要。RL 极已随语料增长大幅补实（reinforcement-learning 入库 ~39 篇，含做市/执行/对冲/持续学习等正例）。
...
```mermaid
flowchart LR
  subgraph SUP["监督预测 (损失钉在收益率/排序, 成本推到外部)"]
    direction TB
    A["StockFormer"]
    B["xLSTM-Mixer"]
    C["DoubleAdapt"]
  ...
  subgraph RL["RL 策略搜索 (成本/时点吞进 reward, 依赖模拟器)"]
    direction TB
    E["MacroHFT"]
    F["DeepAries"]
    G["OPHR 波动率对冲"]
  ...
  SEAM["缝合带: RL 收缩为搜索器, 规避 sim-to-real<br/>Alpha2 / AlphaSAGE / Alpha-R1"]
  SUP -->|"方向先验约束探索"| SEAM
  RL -->|"RL 当因子筛选器"| SEAM
```
