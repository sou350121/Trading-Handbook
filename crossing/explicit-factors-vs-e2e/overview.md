...
**Status:** v0.7 — Opus 手寫綜合，非摘要。「可解释的端到端」缝合带已随语料增长补实（factor-mining 入库 ~56 篇，LLM 驱动的公式因子挖掘成显学）。
...
下图把两轴摊开：x 轴是**解释性/可归因**，y 轴是**拟合上限**。纯公式因子在左上（可读但封顶），纯端到端在右下角对面（拟合强但黑盒），而手册最有价值的「可解释的端到端」缝合体（LLM 挖公式因子、可解释图网络）落在**右上象限**——两个优点都想要。

```mermaid
quadrantChart
  title 可解释因子 vs 端到端黑盒
  x-axis "黑盒 (归因失败)" --> "可解释 (公式可读)"
  y-axis "拟合上限低" --> "拟合上限高"
  quadrant-1 "缝合带: 可解释的端到端"
  quadrant-2 "纯端到端黑盒"
  quadrant-3 "弱可解释低拟合"
  quadrant-4 "经典显式因子"
  "Street PE": [0.88, 0.2]
  "AlphaEvolve": [0.78, 0.32]
  "StockFormer": [0.18, 0.82]
  "THGNN": [0.12, 0.78]
  "FinGAT": [0.15, 0.7]
  "AlphaForge": [0.62, 0.66]
  "AlphaAgent": [0.7, 0.6]
  "NeuralFactors": [0.55, 0.72]
  "TIMEVIEW": [0.58, 0.55]
```
