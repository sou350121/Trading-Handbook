...
**Status:** v0.7 — Opus 手寫綜合，非摘要。微观流极已隨语料增长补实（market-microstructure 入库 ~29 篇）。
...
下图把两个相差几个数量级的时间常数放在一起：静态图（周/月级、容量大、可解释）回答「配多少权重给谁」，动态流（秒/毫秒级、容量极小、难归因）回答「此刻该不该进」。注意缝合带刻意**不融成一张图**，而是按职责分层——语料里真正薄的不是任一极，是这条「图×流」跨层接口。

```mermaid
flowchart LR
  subgraph GRAPH["静态关联图 (周/月级, 容量大, 可解释)"]
    direction TB
    A["GATanalysts 分析师覆盖网络"]
    B["RSR 时序关系排序"]
    C["HATS 分层图注意力"]
    D["NGAT 长期趋势"]
  end
  subgraph FLOW["动态微观流 (秒/毫秒级, 容量极小, 难归因)"]
    direction TB
    E["HLOB 信息持久性"]
    F["COI 交易共现 (1ms)"]
    G["ClusterLOB 订单流聚类"]
  ...
  GRAPH -->|"定权重: 配多少给谁"| SEAM
  FLOW -->|"定时机: 此刻该不该进"| SEAM
  SEAM["职责分层接缝 (语料最薄处)<br/>GAT 组合优化 / 排名空间统计套利 / DGRCL"]
```
