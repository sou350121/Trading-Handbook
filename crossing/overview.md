...
**Status:** v0.6 — Opus 手寫綜合。每张图含五轴投影 + 判别维度对比 + 何时崩 + 代表方法（链接到 Foundations 解构页）。六张图的代表方法已随全量语料（399 篇）补实，每极 4-8 个真实可链方法，（待补）占位已清。
...
- **「代表方法」**链接到 Foundations 解构页；少数解构页尚未建好的，以「注册行」形式列出（zone + msgid，不挂死链）。
