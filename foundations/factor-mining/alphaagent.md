...
**解讀**：基線數值未披露，Δ欄留白。導讀強調的「IC穩定在0.02至0.025」與傳統因子「從0.022-0.036下降至接近零」形成對比，~~此為真實的抗衰减能力體現~~ ⚠️ **見下方 5.1 更正**。但需注意：回測採用top-k剔除與計入交易成本，未披露具體剔除比例與滑點模型，IR與AR的優勢可能部分來自於未充分定價的流動性溢價或樣本內數據挖掘殘留；LLM評估環節的隨機性未做多次種子驗證，存在評估偏差風險。

**5.1 · 更正（2026-07-22，逐字核對 arXiv 2502.16789v2）**

**(a) 上一段的「抗衰減能力體現」應撤回。** 那條「傳統因子 IC 從 0.022-0.036 降至接近零」的五年衰減對照，以及「AlphaForge 3.45% / RD-Agent 0.78%」這組競爭系統成績，在三票對抗驗證下**皆未通過（0-3）**——它們是**利害關係方報告的競爭對手數字**，無獨立來源。正確措辭是「論文自述，未經獨立驗證」，不是「真實能力體現」。

**(b) 成本模型比「計入交易成本」這句話薄得多。** Table 2 逐字：CSI 500 買 0.05% / 賣 0.15%；**S&P 500 只收賣出 0.05%——買入免費，且無價差、無市場衝擊、無借券成本。** 這使 S&P 500 那一行的 AR 8.74% / IR 1.0545 建立在一個不會存在的成本假設上。

**(c) 最尖銳的內部張力**：S&P 500 的 **IC 僅 0.0056**（接近零的截面預測力），卻宣稱 IR 1.0545。這個組合只有在成本近乎為零時才可能成立——而它的成本模型正好近乎為零。

**(d) 統計控制**：唯一的控制是對 RD-Agent 的兩兩 t 檢定，p = 0.031 / 0.011 / 0.038——這種邊際在天真的多重性校正下即失效。全文無 deflated Sharpe / PBO / data snooping 校正。搜尋批評或複現只找到論文本身、作者 GitHub 與把它當 baseline 的後續工作（Chain-of-Alpha、Alpha Jungle MCTS、EFS），**零獨立驗證**。

> 仍然成立的部分：AST 相似度懲罰作為**反擁擠新穎性正則**的設計，以及論文對「LLM 生成缺乏同質化正則」的診斷。那是關於機制的論證，不是關於績效的證據。詳見 [審計報告](/reports/deep-research/2026-07-22-autoresearch-audit)。
