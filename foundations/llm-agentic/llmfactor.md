...
```mermaid
flowchart TD
    A["News + Stock Target"] --> B["[Stage 1: Fill-in-the-blank]"]
    B --> C["Company/Industry Background Knowledge"]
    B --> D["[Stage 2: Directed Extraction]"]
    D --> E["Market-Driving Factors (Structured)"]
    D --> F["[Stage 3: Seq-to-Text + Joint Reasoning]"]
    F --> G["Historical Price Sequence (Text) + Factors"]
    G --> H["Predict ↑/↓ + Rationale"]
```
