...
**1.3 信息流 ASCII 圖**
```
Raw Data -> [Data Agents] -> Textual Factors (≤4k tokens)
                      |
                      v
              [Contest 1: Quantify-Predict-Allocate]
                      | (ZI Trader -> LightGBM -> 0/1 Knapsack)
                      v
          Optimized Factor Portfolio (≤16k tokens)
                      |
                      v
          [Research Agents] -> Trading Signals (Symbol, Action, Evidence, Limitation)
                      |
                      v
              [Contest 2: Quantify-Predict-Allocate]
                      | (Realized Sharpe + LLM Judger -> LightGBM -> Pred. Sharpe Weight)
                      v
              Final Capital Allocation / Final Signals
```
