...
**1.3 信息流:**
```
Raw OHLCV → [波動模式分類器] → FinTSB Dataset (4 regimes)
      ↓
[統一訓練層] → Loss = MSE/MAE + Rank Loss → 預測日收益率
      ↓
[回測交易協議] → 預測排名 → TopK-Drop (K=5, 10% turnover) → 扣 0.1% 手續費
      ↓
[三維指標輸出] → Error / Rank / Portfolio Metrics → 反饋調優
```
