...
> **原始論文**：[Kolmogorov-Arnold Networks for Time Series Analysis and Forecasting: A Survey](https://doi.org/10.2139/ssrn.5220118)（2025 · 被引 0 · Crossref）
...
```mermaid
flowchart TD
    A["原始序列 X_t"] -->|"(移動平均分層預處理)"| B["多級序列 {x_0, x_1, ..., x_L}"]
    B -->|"(CFD: FFT → Padding → IFFT → 殘差提取)"| C["頻帶表示 {f_1, f_2, ..., f_L}"]
    C -->|"(M-KAN: 多階 Chebyshev 通道學習 + Depthwise Conv 時序依賴)"| D["頻帶輸出 {o_1, o_2, ..., o_L}"]
    D -->|"(頻率混合: 逆 FFT 上採樣 逐步疊加)"| E["最高級序列 x_L → 線性層 → 預測 Y_{t+1:t+H}"]
```
