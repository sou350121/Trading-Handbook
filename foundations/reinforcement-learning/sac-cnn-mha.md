...
**信息流:**
```
[市場數據 1h] → (滑窗 48h) → [張量 M×4×48]
          ↓
    [CNN 特徵提取] → [MHA 跨資產注意力] → [狀態 S_t]
          ↓
[SAC Actor] → N(μ, σ) → 抽樣/取眾數 → [權重 W_t ∈ [-1,1]^M, 貸款]
          ↓
[Env Step] → 計算 PnL (扣 δ, 計 ζ) → [獎勵 R_t, 狀態 S_{t+1}]
```
