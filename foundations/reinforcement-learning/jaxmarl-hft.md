...
flowchart TD
    N1["數據載入(GPU)"] --> N2["片段劃分"]
    N2["片段劃分"] --> N3["vmap並行(跨片段/跨類型)"]
    N2["片段劃分"] --> N5["隨機Shuffling"]
    N4["動作轉換"] --> N5["隨機Shuffling"]
    N5["隨機Shuffling"] --> N6["消息增強(歷史流+智能體)"]
    N5["隨機Shuffling"] --> N8["獎勵/觀察計算"]
    N7["JAX-LOB處理"] --> N8["獎勵/觀察計算"]
    N8["獎勵/觀察計算"] --> N9["返回梯度/狀態"]
```
