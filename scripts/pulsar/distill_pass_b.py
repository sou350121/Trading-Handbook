...
【硬規則】導讀正文沒有明確給出的數字一律寫「未披露」，**禁止臆造任何數值、百分比或區間**（含 §5 benchmark 表：沒有的 Metric/Δ 寫「未披露」，絕不寫類似「~1.2-1.8」「+40%」的猜測值）。寧可整列空白也不要編數字。
...
**TL;DR:**（≤4 句：①做什麼新事 ②核心 trick ③為什麼這對某條軸★ ④一個關鍵實證數字）
...
## §5 · 評測 / Benchmark
（表：數據集/市場 | Metric(IR/Sharpe/AR/MDD) | 前SOTA | 本方法 | Δ；解讀哪部分 Δ 是真 capability，哪部分可能是過擬合/前瞻偏差/成本未計）
